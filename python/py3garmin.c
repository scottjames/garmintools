...
static PyObject *
toggle_verbose(PyObject *obj, PyObject *args)
{
  verbose = (int)!verbose;
...
static PyObject *
get_verbose(PyObject *obj, PyObject *args)
{
  return PyBool_FromLong(verbose);
}
...
bool
initialize_garmin(garmin_unit *garmin)
{
...
static PyObject *
get_info(PyObject *obj, PyObject *args)
{
  garmin_unit garmin;
  ...
  PyDict_SetItem(
    dict, PyUnicode_FromString("unit_id"), PyLong_FromUnsignedLong(unit_id));

  uint16 product_id = garmin.product.product_id;
  PyDict_SetItem(
    dict, PyUnicode_FromString("product_id"), PyLong_FromLong(product_id));

  double software_version = garmin.product.software_version / 100.0;
  PyDict_SetItem(dict,
                 PyUnicode_FromString("software_version"),
                 PyFloat_FromDouble(software_version));

  char *product_description = garmin.product.product_description;
  PyDict_SetItem(dict,
                 PyUnicode_FromString("description"),
                 PyUnicode_FromString(product_description));
...
static PyObject *
get_runs(PyObject *obj, PyObject *args)
{
  garmin_unit garmin;
  ...
  */
  ...
  if (tmpdata == NULL) {
    PyErr_SetString(PyExc_RuntimeError,
                    "Toplevel data missing element 0 (runs)");
    goto out;
  }
  ...
  if (tmpdata == NULL) {
    PyErr_SetString(PyExc_RuntimeError,
                    "Toplevel data missing element 1 (laps)");
    goto out;
  }
  ...
  if (tmpdata == NULL) {
    PyErr_SetString(PyExc_RuntimeError,
                    "Toplevel data missing element 2 (tracks)");
    goto out;
  }
  ...
  for (n = runs->head; n != NULL; n = n->next) {
    if (get_run_track_lap_info(n->data, &trk, &f_lap, &l_lap) != 0) {
      ...
      PyDict_SetItem(
        run, PyUnicode_FromString("track"), Py_BuildValue("i", trk));
      PyDict_SetItem(
        run, PyUnicode_FromString("first_lap"), Py_BuildValue("i", f_lap));
      PyDict_SetItem(
        run, PyUnicode_FromString("last_lap"), Py_BuildValue("i", l_lap));
      PyDict_SetItem(run,
                     PyUnicode_FromString("type"),
                     Py_BuildValue("i", (int)n->data->type));
      ...
      if (n->data->type == data_D1009) {
        ...
        d1009 = n->data->data;
        PyDict_SetItem(run,
                       PyUnicode_FromString("multisport"),
                       PyBool_FromLong(d1009->multisport));
        switch (d1009->sport_type) {
        case D1000_running:
          PyDict_SetItem(run,
                         PyUnicode_FromString("sport"),
                         PyUnicode_FromString("running"));
          break;
        case D1000_biking:
          PyDict_SetItem(
            run, PyUnicode_FromString("sport"), PyUnicode_FromString("biking"));
          break;
        case D1000_other:
          PyDict_SetItem(
            run, PyUnicode_FromString("sport"), PyUnicode_FromString("other"));
          break;
        }
      }
        ...
        if (get_lap_index(m->data, &l_idx) != 0) {
          if (l_idx >= f_lap && l_idx <= l_lap) {
            ...
            if (start != 0) {
              ...
              PyDict_SetItem(lap,
                             PyUnicode_FromString("start_time"),
                             Py_BuildValue("i", (int)start));
              PyDict_SetItem(lap,
                             PyUnicode_FromString("type"),
                             Py_BuildValue("i", (int)m->data->type));

              if (m->data->type == data_D1015) {
                D1015 *d1015;
                d1015 = m->data->data;
                PyDict_SetItem(lap,
                               PyUnicode_FromString("duration"),
                               Py_BuildValue("i", d1015->total_time));
                PyDict_SetItem(lap,
                               PyUnicode_FromString("distance"),
                               Py_BuildValue("f", d1015->total_dist));
                PyDict_SetItem(lap,
                               PyUnicode_FromString("max_speed"),
                               Py_BuildValue("f", d1015->max_speed));
              }
              ...
              for (o = tracks->head; o != NULL; o = o->next) {
                if (o->data != NULL) {
                  if (o->data->type == data_D311) {
                    ...
                    if (d304->posn.lat != 2147483647 &&
                        d304->posn.lon != 2147483647) {
                      PyDict_SetItem(point,
                                     PyUnicode_FromString("position"),
                                     Py_BuildValue("(ff)",
                                                   SEMI2DEG(d304->posn.lat),
                                                   SEMI2DEG(d304->posn.lon)));
                      PyDict_SetItem(point,
                                     PyUnicode_FromString("type"),
                                     Py_BuildValue("i", (int)o->data->type));
                      PyDict_SetItem(
                        point,
                        PyUnicode_FromString("time"),
                        Py_BuildValue("f", (float)(d304->time + TIME_OFFSET)));
                      PyDict_SetItem(point,
                                     PyUnicode_FromString("distance"),
                                     Py_BuildValue("f", d304->distance));
                      PyDict_SetItem(point,
                                     PyUnicode_FromString("altitude"),
                                     Py_BuildValue("f", d304->alt));
                      PyDict_SetItem(point,
                                     PyUnicode_FromString("heart_rate"),
                                     Py_BuildValue("i", d304->heart_rate));

                      if (d304->cadence != 255)
                        PyDict_SetItem(point,
                                       PyUnicode_FromString("cadence"),
                                       Py_BuildValue("i", d304->cadence));
              ...
              PyDict_SetItem(lap,
                             PyUnicode_FromString("points"),
                             Py_BuildValue("N", points));
            } else
              PyErr_Warn(PyExc_Warning, "Start time of first lap not found.");

            PyDict_SetItem(rlaps,
                           PyUnicode_FromFormat("%d", (int)l_idx),
                           Py_BuildValue("N", lap));
          }
        }
      }
      PyDict_SetItem(
        run, PyUnicode_FromString("laps"), Py_BuildValue("N", rlaps));
      PyDict_SetItem(dict,
                     PyUnicode_FromFormat("%d", (int)f_lap_start),
                     Py_BuildValue("N", run));
    }
  }
...
static PyMethodDef MethodTable[] = {
  {"toggle_verbose",
   toggle_verbose,
   METH_VARARGS,
   "Toggle verbose flag and return its new state, True if turned on, False "
   "else."},
  {"get_verbose",
   get_verbose,
   METH_VARARGS,
   "Return the current state of the verbose flag, True if turned on, False "
   "else."},
  {"get_info",
   get_info,
   METH_VARARGS,
   "Return a dictionary with information about the attached unit."},
  {"get_runs",
   get_runs,
   METH_VARARGS,
   "Return a dictionary with all runs stored on the attached unit."},
  {NULL, NULL, 0, NULL}};

static struct PyModuleDef moduledef = {
  PyModuleDef_HEAD_INIT, "pygarmin", NULL, -1, MethodTable};

/* Initialize pygarmin as python extension */

PyMODINIT_FUNC
PyInit_pygarmin(void)
{
  PyObject *module = 0;
